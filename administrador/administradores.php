...
if(isset($_COOKIE['VotaDatAdmin'])) {
	...
	if (isset($_POST['envia_admin'])) {
		...
		$duplica=0;
		$resp3=$link->query("select usuario from administradores");
...
$link->query($cons_sql5);
	...
	if((isset($_GET['agrega']))and($_GET['agrega']=="ok")) { 
		echo '<form name="addadmin" action="administradores.php" method="post">';
	        ...
	        echo '</label></td>';
	        echo '<td><input type="password" name="clave_adm" size="30" maxlength="30" title="Escriba la contrase�a de acceso" />';
	        echo '</td></tr>';
	...
	if((isset($_GET['id'])) and ($_GET['id']!=md5(1)) and (isset($_GET['editar'])) and ($_GET['editar']=="ok")) { 
		$resp4=$link->query(sprintf("select * from administradores where md5(id)=%s",comillas($_GET['id'])));
	...
	if((isset($_GET['id']))and($_GET['id']!=md5(1))and(isset($_GET['elimina']))and($_GET['elimina']=="0")) {
		...
		$resp5=$link->query(sprintf("select usuario from administradores where md5(id)=%s",comillas($_GET['id'])));
			...
			echo '<a href="administradores.php" title="Cancelar la eliminaci�n del administrador">No</a>';
			echo '</strong></div>';
		}
		else {
			echo '<table>';
	...
	if((isset($_GET['id']))and($_GET['id']!=md5(1))and(isset($_GET['elimina']))and($_GET['elimina']=="1")) {
		$resp6=$link->query(sprintf("select usuario from administradores where md5(id)=%s",comillas($_GET['id'])));
	        $row6 = $resp6->fetch_array();
		$resp2=$link->query(sprintf("delete from administradores where md5(id)=%s",comillas($_GET['id'])));
...
$link->query($cons_sql5);
	...
	$ContAdm=0;
	$resp=$link->query(sprintf("select * from administradores order by nombres"));
